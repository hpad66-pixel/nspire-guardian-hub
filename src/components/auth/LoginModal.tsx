...
export function LoginModal({ open, onOpenChange }: LoginModalProps) {
  ...
  const handleAuth0SignIn = async () => {
    ...
    try {
      const { error } = await signInWithAuth0(`${window.location.origin}/dashboard`);
      ...
      >
        ...
        >
          ...
          <p className="mb-6 text-[13.5px] text-white/50">
            Sign in to your APAS Project Controls workspace.
          </p>

          <div className="mb-4 space-y-3">
            {isAuth0SignInEnabled() && (
              <button
                onClick={handleAuth0SignIn}
                disabled={isAuth0Loading}
                className="flex h-11 w-full items-center justify-center gap-3 rounded-xl border border-white/[0.12] bg-white/[0.10] text-[14px] font-semibold text-white transition-colors hover:bg-white/[0.14] disabled:opacity-60"
              >
                {isAuth0Loading ? (
            ...
            <button
              ...
              className="flex h-11 w-full items-center justify-center gap-3 rounded-xl border border-white/[0.08] bg-white/[0.04] text-[14px] font-medium text-white/90 transition-colors hover:bg-white/[0.07] disabled:opacity-60"
            >
              {isGoogleLoading ? (
