...
/** Construction — full money lifecycle (pay apps, budget, commitments…). */
const CONSTRUCTION_PRIMARY: Tab[] = [
...
/** Consulting / client — proposal → branded invoice → cash. No pay apps. */
const CONSULTING_PRIMARY: Tab[] = [
  { label: "Overview", path: "overview", icon: LayoutDashboard },
  { label: "Proposals", path: "proposals", icon: FileText },
...
export function FinancialSubNav() {
  ...
  const PRIMARY = isConsulting ? CONSULTING_PRIMARY : CONSTRUCTION_PRIMARY;
  const MORE_GROUPS = isConsulting ? CONSULTING_MORE : CONSTRUCTION_MORE;
