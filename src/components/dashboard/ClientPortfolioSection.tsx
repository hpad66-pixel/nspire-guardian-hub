      ...
      <div className="grid grid-cols-1 gap-3 lg:grid-cols-2 xl:grid-cols-3">
        {groups.map((group) => {
          ...
          return (
            <article
              key={group.id}
              className="rounded-2xl border border-border/70 bg-card p-3.5 shadow-sm transition hover:border-primary/25 hover:shadow-md"
            >
              <div className="flex items-start justify-between gap-3">
                <button type="button" onClick={() => navigate(target)} className="min-w-0 text-left">
                  <p className="truncate font-display text-base font-bold leading-tight">{group.name}</p>
                  <p className="mt-1 text-xs text-muted-foreground">
                    {group.projects.length} project{group.projects.length === 1 ? '' : 's'} | {activeCount} active | {lockedCount} locked
                  </p>
                </button>
                <Button size="sm" variant="outline" className="h-7 shrink-0 gap-1 px-2.5 text-xs" onClick={() => navigate(target)}>
              ...
              <div className="mt-3 grid grid-cols-2 gap-2">
                ...
                <button
                  ...
                  className="rounded-xl border border-[var(--kind-consulting-accent)]/25 bg-[var(--kind-consulting)] p-2.5 text-left text-[var(--kind-consulting-ink)] transition hover:-translate-y-0.5 hover:border-[var(--kind-consulting-accent)]/45 hover:shadow-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--kind-consulting-accent)]/35"
                  aria-label={`Open ${group.name} consulting projects`}
                >
                  <div className="flex items-center gap-2">
                    <Briefcase className="h-3.5 w-3.5" />
                    <span className="text-[11px] font-bold uppercase tracking-wide">Consulting</span>
                  </div>
                  <div className="mt-1.5 flex items-end justify-between gap-2">
                    <p className="text-xl font-black tabular-nums">{kinds.consulting.length}</p>
                    <ArrowRight className="h-3.5 w-3.5 opacity-70" />
                  </div>
                </button>
              </div>
              ...
              <div className="mt-3 flex flex-wrap gap-1.5">
                {protectedCount > 0 && (
                  <Badge variant="outline" className="gap-1 border-amber-300 bg-amber-50 px-2 py-0.5 text-[10px] text-amber-900">
                    <LockKeyhole className="h-3 w-3" />
                    {protectedCount} protected historical
                  </Badge>
                )}
                {lockedCount > 0 && (
                  <Badge variant="outline" className="gap-1 border-slate-300 bg-slate-50 px-2 py-0.5 text-[10px] text-slate-700">
                    <CheckCircle2 className="h-3 w-3" />
                    {lockedCount} locked
                  </Badge>
                )}
                {activeCount > 0 && (
                  <Badge variant="outline" className="gap-1 border-primary/25 bg-primary/5 px-2 py-0.5 text-[10px] text-primary">
              ...
              <div className="mt-3 space-y-1.5">
                {group.projects.slice(0, 2).map((project) => (
                  <button
                    ...
                    className="flex w-full items-center justify-between gap-3 rounded-lg border border-transparent bg-muted/35 px-2.5 py-1.5 text-left text-sm transition hover:border-primary/20 hover:bg-muted/55"
                  >
                ...
                {group.projects.length > 2 && (
                  <button
                    ...
                    className="flex w-full items-center justify-center gap-1.5 rounded-lg border border-dashed border-border/80 px-2.5 py-1.5 text-xs font-semibold text-muted-foreground transition hover:border-primary/35 hover:bg-primary/5 hover:text-primary"
                  >
                    {group.projects.length - 2} more project{group.projects.length - 2 === 1 ? '' : 's'}
