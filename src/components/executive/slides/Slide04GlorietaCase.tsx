      ...
      <div className="slide-content">
        ...
        <div className="flex gap-6 flex-1 mt-8">
          ...
          <motion.div variants={containerVariants} initial="hidden" animate="visible"
            className="flex-1 rounded-2xl bg-[#111E38] border border-white/5 p-6 flex flex-col gap-3">
            <motion.div variants={itemVariants} className="text-[8px] font-bold tracking-widest text-[#10B981]">PROPERTY SNAPSHOT</motion.div>
            {[
              ...
              <motion.div key={k} variants={itemVariants} className="flex gap-3">
                <span className="text-[10px] text-[#6B7A99] w-24 flex-shrink-0">{k}:</span>
          ...
          <motion.div variants={containerVariants} initial="hidden" animate="visible" className="flex-1 grid grid-cols-2 gap-3">
            {metrics.map((m) => (
              <motion.div key={m.label} variants={itemVariants}
                className="rounded-xl bg-[#111E38] border border-white/5 p-5 flex flex-col gap-1">
                {isLoading ? (
                  <Skeleton className="h-10 w-16 bg-white/5" />
                ) : (
                  <div className="text-[28px] font-bold" style={{ color: m.color }}>{m.n}</div>
                )}
                <div className="text-[11px] font-semibold text-white">{m.label}</div>
                <div className="text-[9px] text-[#6B7A99]">{m.sub}</div>
              </motion.div>
            ))}
          </motion.div>
        </div>
