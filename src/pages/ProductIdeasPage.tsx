...
function ProgressTickler({ status, condensed = false }: { status: ProductIdeaStatus; condensed?: boolean }) {
  const activeIndex = productIdeaProgressIndex(status);
  const rejected = status === 'rejected';

  return (
    <div className={cn('w-full', condensed ? 'max-w-[220px]' : '')}>
        ...
        {PRODUCT_IDEA_PROGRESS.map((stage, index) => {
          ...
          return (
            <div key={stage.key} className={cn('flex items-center', index < PRODUCT_IDEA_PROGRESS.length - 1 && 'flex-1')}>
              <div
                className={cn(
                  'flex shrink-0 items-center justify-center rounded-full border-2 bg-background transition-colors',
              ...
              {index < PRODUCT_IDEA_PROGRESS.length - 1 && (
                <div
                  className={cn(
                    'h-0.5 flex-1',
                    index < activeIndex && !rejected ? 'bg-emerald-500' : 'bg-border',
                  )}
                />
              )}
            </div>
...
function IdeaCard({ idea, onOpen }: { idea: ProductIdea; onOpen: () => void }) {
  ...
  return (
    <article
      ...
      className="group grid cursor-pointer grid-cols-[auto_1fr] gap-4 rounded-2xl border border-border/70 bg-card p-4 shadow-sm transition-all hover:-translate-y-0.5 hover:border-primary/25 hover:shadow-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/30 sm:p-5 lg:grid-cols-[auto_1fr_auto]"
    >
      <VoteControl idea={idea} />
      <div className="min-w-0">
        ...
        <h3 className="text-lg font-bold tracking-[-0.02em] text-foreground transition-colors group-hover:text-primary">
          {idea.title}
        </h3>
      ...
      <div className="col-span-2 flex min-w-[220px] items-center gap-3 border-t pt-4 lg:col-span-1 lg:flex-col lg:items-end lg:border-l lg:border-t-0 lg:pl-5 lg:pt-0">
...
function AdminUpdateDialog({
  ...
  const [status, setStatus] = useState<ProductIdeaStatus>(idea.status);
  const [title, setTitle] = useState('');
  ...
  const submit = async () => {
    ...
    try {
      await publish.mutateAsync({ ideaId: idea.id, status, title, body });
      setTitle('');
      setBody('');
      onOpenChange(false);
    } catch {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-xl">
        <DialogHeader>
          <DialogTitle>Publish a client update</DialogTitle>
          <DialogDescription>
            Move this idea forward or explain the decision. The update becomes part of its permanent client-facing history.
          </DialogDescription>
        </DialogHeader>
        <div className="space-y-4 py-2">
          <div className="space-y-2">
            <label className="text-sm font-semibold">Current position</label>
          ...
          <div className="space-y-2">
            <label htmlFor="update-title" className="text-sm font-semibold">Update headline</label>
            <Input
              id="update-title"
              value={title}
              onChange={(event) => setTitle(event.target.value)}
    ...
    <>
      <Sheet open={open} onOpenChange={onOpenChange}>
        <SheetContent className="w-full overflow-y-auto p-0 sm:max-w-2xl">
          <div className="border-b bg-gradient-to-br from-primary/[0.08] via-background to-amber-500/[0.07] px-6 pb-6 pt-8 sm:px-8">
          ...
          <div className="space-y-8 px-6 py-7 sm:px-8">
            ...
            <section className="rounded-2xl border bg-muted/20 p-5">
              <div className="mb-5 flex items-center justify-between gap-3">
                <div>
                  ...
                  <p className="mt-0.5 text-xs text-muted-foreground">Updated by the Proj OS product team</p>
                </div>
                {isAdmin && (
              ...
              <ProgressTickler status={idea.status} />
            </section>
            ...
            <section>
              <div className="mb-5 flex items-end justify-between">
                <div>
                  <p className="text-[11px] font-bold uppercase tracking-[0.14em] text-muted-foreground">Update history</p>
                  <h3 className="mt-1 text-lg font-bold">From request to decision</h3>
                </div>
                ...
                {idea.updates.map((update, index) => {
                  ...
                  return (
                    <div key={update.id} className="relative pb-7">
                      ...
                      <p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-foreground/80">{update.body}</p>
      ...
      <AdminUpdateDialog idea={idea} open={adminOpen} onOpenChange={setAdminOpen} />
    </>
  );
}
          ...
          <div className="grid gap-8 lg:grid-cols-[1fr_auto] lg:items-end">
            <div className="max-w-3xl">
              ...
              <p className="mt-4 max-w-2xl text-base leading-7 text-white/72 sm:text-lg">
                Share the capability your team needs, support ideas from other clients, and follow every decision from review to release.
              </p>
              <Button
                size="lg"
            ...
            <div className="grid grid-cols-2 gap-2 sm:grid-cols-4 lg:grid-cols-2">
              {[
                ...
                { label: 'Released', value: metrics.shipped, icon: CheckCircle2 },
            ...
            <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_300px]">
              <div className="rounded-2xl border bg-card p-5 shadow-sm sm:p-7">
                <div className="mb-6">
                  ...
                  <p className="mt-2 text-sm leading-6 text-muted-foreground">Every escalation, roadmap move, release, and decision from the Proj OS team.</p>
                </div>
                {updateFeed.length === 0 ? (
              ...
              <aside className="space-y-4">
                ...
                <div className="rounded-2xl border bg-card p-5 shadow-sm">
                  ...
                  <div className="mt-4 space-y-4 text-sm">
                    ...
                    <div className="flex gap-3"><CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0 text-emerald-500" /><p><strong>Released</strong><br /><span className="text-muted-foreground">The capability is available.</span></p></div>
                  </div>
                </div>
              </aside>
            </div>
