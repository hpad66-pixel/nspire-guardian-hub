            ...
            <CardContent className="p-6">
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-muted-foreground">{formatCurrency(spent)} spent ({spentProgress}%)</p>
                </div>
              </div>
                ...
                <div className="flex-1">
                  ...
                  {daysRemaining !== null ? (
                    <>
                      ...
                      <p className="text-xs text-muted-foreground">Target: {format(new Date(project.target_end_date!), 'MMM d, yyyy')}</p>
                    </>
                  ) : (
                    <><p className="text-2xl font-bold">No deadline</p><p className="text-xs text-muted-foreground">Set a target end date</p></>
                  )}
                </div>
              </div>
            </CardContent>
        ...
        <Tabs defaultValue="overview" className="space-y-6">
          <TabsList className="flex-wrap h-auto gap-1">
            ...
            <TabsTrigger value="rfis" className="gap-1">
              RFIs
              {(rfiStats?.open ?? 0) > 0 && (
                <Badge variant="secondary" className="ml-1 h-5 px-1.5 text-xs">{rfiStats?.open}</Badge>
              )}
            </TabsTrigger>
          ...
          <TabsContent value="overview" className="space-y-6">
              ...
              <Card>
                ...
                <CardContent>
                  <div className="space-y-4">
                    {project.description && (
                      <div><h4 className="text-sm font-medium mb-1">Description</h4><p className="text-sm text-muted-foreground">{project.description}</p></div>
                    )}
                    {project.scope && (
                      <div><h4 className="text-sm font-medium mb-1">Scope Details</h4><p className="text-sm text-muted-foreground whitespace-pre-wrap">{project.scope}</p></div>
                    )}
                ...
                <CardContent>
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-sm font-medium">{project.start_date ? format(new Date(project.start_date), 'MMM d, yyyy') : 'Not set'}</span>
                    </div>
                    <div className="flex items-center justify-between">
                      <span className="text-sm text-muted-foreground">Target End Date</span>
                      <span className="text-sm font-medium">{project.target_end_date ? format(new Date(project.target_end_date), 'MMM d, yyyy') : 'Not set'}</span>
                    </div>
                    <div className="flex items-center justify-between">
                      <span className="text-sm text-muted-foreground">Actual End Date</span>
                      <span className="text-sm font-medium">{project.actual_end_date ? format(new Date(project.actual_end_date), 'MMM d, yyyy') : 'In progress'}</span>
                    </div>
                    <div className="flex items-center justify-between">
                      <span className="text-sm text-muted-foreground">Created</span>
                      <span className="text-sm font-medium">{format(new Date(project.created_at), 'MMM d, yyyy')}</span>
                    </div>
                  </div>
                </CardContent>
              </Card>
            ...
            <div className="grid gap-4 md:grid-cols-4">
              <Card><CardContent className="p-4 text-center"><p className="text-3xl font-bold">{totalMilestones}</p><p className="text-sm text-muted-foreground">Milestones</p></CardContent></Card>
              <Card><CardContent className="p-4 text-center"><p className="text-3xl font-bold">{dailyReports?.length || 0}</p><p className="text-sm text-muted-foreground">Daily Reports</p></CardContent></Card>
              <Card><CardContent className="p-4 text-center"><p className="text-3xl font-bold">{changeOrders?.length || 0}</p><p className="text-sm text-muted-foreground">Change Orders</p></CardContent></Card>
              <Card><CardContent className="p-4 text-center"><p className="text-3xl font-bold">{formatCurrency(approvedCOAmount)}</p><p className="text-sm text-muted-foreground">Approved COs</p></CardContent></Card>
            </div>
          </TabsContent>
          ...
          <TabsContent value="financials">
            <ProjectFinancials project={project} changeOrders={changeOrders || []} />
          </TabsContent>
          <TabsContent value="rfis">
            <RFIList projectId={id!} />
          </TabsContent>
          <TabsContent value="submittals">
            <SubmittalsTab projectId={id!} />
          </TabsContent>
