...
import {
  buildConsultingReportHtml, openConsultingReport, prepareConsultingReportDelivery,
  removeLongDashes,
...
function ReportWorkspace({ report, projectId, projectName, onBack, reports }: { report: ConsultingReport; projectId: string; projectName: string; onBack: () => void; reports: ReportsHook }) {
  ...
  const includedSources = (sources.data ?? []).filter((source) => source.included);
  const previewHtml = useMemo(() => buildConsultingReportHtml({ projectName, report: currentReport, sources: sources.data ?? [], imageUrls: Object.fromEntries((sources.data ?? []).filter((source) => source.preview_url).map((source) => [source.id, source.preview_url!])) }), [currentReport, projectName, sources.data]);
  ...
  async function generateDraft() {
    ...
    try {
      const { data, error } = await supabase.functions.invoke('consulting-report-ai', { body: { operation: 'generate', projectId, reportId: report.id, conversation } });
      if (error) throw error;
      if (data?.error) throw new Error(data.error);
      ...
      await reports.update.mutateAsync({ id: report.id, title: nextTitle, subtitle: nextSubtitle || null, body_html: nextBody, generation_notes: { editorNote: data.editorNote, model: data.model, generatedAt: new Date().toISOString(), sourceCount: includedSources.length }, status: 'draft' });
      setTab('draft');
  ...
  async function addFiles(files: FileList | null) {
    ...
    try {
      await sources.upload.mutateAsync(Array.from(files));
  ...
  return (
    <div className="min-h-[calc(100vh-5rem)] bg-[#f4f6f4]">
      <input ref={uploadRef} type="file" multiple accept={SOURCE_ACCEPT} className="hidden" onChange={(event) => { void addFiles(event.target.files); event.target.value = ''; }} />
      <header className="sticky top-0 z-20 border-b bg-[#071f19]/95 text-white shadow-sm backdrop-blur">
        <div className="mx-auto flex max-w-[1600px] flex-wrap items-center gap-3 px-4 py-3 sm:px-6">
          ...
          <Button size="sm" className="bg-amber-300 font-bold text-amber-950 hover:bg-amber-200" onClick={() => void generateDraft()} disabled={generating}>{generating ? <Loader2 className="mr-1.5 h-4 w-4 animate-spin" /> : <Sparkles className="mr-1.5 h-4 w-4" />}Generate report</Button>
        </div>
      </header>
      ...
      <main className="mx-auto max-w-[1600px] p-3 sm:p-5">
        <Tabs value={tab} onValueChange={setTab} className="space-y-4">
          <TabsList className="grid h-auto w-full grid-cols-4 rounded-xl bg-white p-1 shadow-sm sm:w-[720px]">
            <TabsTrigger value="conversation" className="gap-2 rounded-lg py-2.5"><MessageSquareText className="h-4 w-4" /><span className="hidden sm:inline">Conversation</span></TabsTrigger>
            <TabsTrigger value="sources" className="gap-2 rounded-lg py-2.5"><Paperclip className="h-4 w-4" /><span className="hidden sm:inline">Sources</span><Badge variant="secondary" className="h-5 px-1.5 text-[10px]">{includedSources.length}</Badge></TabsTrigger>
            <TabsTrigger value="draft" className="gap-2 rounded-lg py-2.5"><FileText className="h-4 w-4" /><span className="hidden sm:inline">Draft</span></TabsTrigger>
            <TabsTrigger value="preview" className="gap-2 rounded-lg py-2.5"><BookOpen className="h-4 w-4" /><span className="hidden sm:inline">Preview</span></TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="sources" className="mt-0">
            <Card><CardContent className="p-5 sm:p-6"><div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-[10px] font-black uppercase tracking-[.16em] text-emerald-700">Private evidence room</p><h2 className="mt-1 font-display text-3xl text-[#082b23]">Report sources and photographs</h2><p className="mt-1 text-sm text-muted-foreground">Only checked records enter the report. Original files stay in private project storage.</p></div><div className="flex flex-wrap gap-2"><Button variant="outline" onClick={() => setDriveOpen(true)}><Cloud className="mr-2 h-4 w-4" />Google Drive</Button><Button onClick={() => uploadRef.current?.click()} disabled={sources.upload.isPending}>{sources.upload.isPending ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Upload className="mr-2 h-4 w-4" />}Upload files</Button></div></div>
              {sources.isLoading ? <div className="grid min-h-52 place-items-center"><Loader2 className="h-5 w-5 animate-spin" /></div> : !(sources.data ?? []).length ? <button type="button" onClick={() => uploadRef.current?.click()} className="mt-6 grid min-h-64 w-full place-items-center rounded-2xl border-2 border-dashed border-emerald-200 bg-emerald-50/30 p-8 text-center transition hover:bg-emerald-50"><div><Upload className="mx-auto h-8 w-8 text-emerald-700" /><h3 className="mt-3 font-semibold text-[#082b23]">Add photographs, manifests, or supporting records</h3><p className="mt-1 text-sm text-muted-foreground">Images, PDF, Word, Excel, CSV, text, and presentation files are accepted.</p></div></button> : <div className="mt-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">{(sources.data ?? []).map((source) => <SourceCard key={source.id} source={source} onUpdate={(patch) => void sources.update.mutateAsync({ id: source.id, ...patch }).catch((error) => toast.error(error.message))} onRemove={() => { if (window.confirm(`Remove “${source.source_name}” from this report?`)) void sources.remove.mutateAsync(source).catch((error) => toast.error(error.message)); }} />)}</div>}
            </CardContent></Card>
          </TabsContent>

          <TabsContent value="draft" className="mt-0">
            <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_320px]"><Card><CardContent className="space-y-4 p-5"><div className="grid gap-4 sm:grid-cols-2"><div className="space-y-2 sm:col-span-2"><Label>Report title</Label><Input value={title} onChange={(event) => setTitle(removeLongDashes(event.target.value))} className="text-base font-semibold" /></div><div className="space-y-2 sm:col-span-2"><Label>Subtitle</Label><Input value={subtitle} onChange={(event) => setSubtitle(removeLongDashes(event.target.value))} /></div><div className="space-y-2"><Label>Report date</Label><Input type="date" value={reportDate} onChange={(event) => setReportDate(event.target.value)} /></div><div className="space-y-2"><Label>Review status</Label><div className="flex h-10 items-center rounded-md border bg-muted/30 px-3 text-sm">{statusMeta[currentReport.status].label}</div></div></div><div className="space-y-2"><div className="flex items-center justify-between"><Label>Report narrative</Label><span className="text-[11px] text-muted-foreground">Fully editable before issue</span></div><ProRichTextEditor content={bodyHtml} onChange={(html) => setBodyHtml(removeLongDashes(html))} placeholder="Generate a draft or begin writing the report..." minHeight="620px" /></div></CardContent></Card><aside className="space-y-3"><div className="rounded-2xl border bg-white p-4"><h3 className="font-semibold text-[#082b23]">Report controls</h3><div className="mt-3 grid gap-2"><Button variant="outline" onClick={() => void saveReport()} disabled={saving}><Save className="mr-2 h-4 w-4" />Save changes</Button><Button variant="outline" onClick={() => { setTab('preview'); void saveReport({ status: 'ready' }); }}><CheckCircle2 className="mr-2 h-4 w-4" />Mark ready and preview</Button><Button onClick={() => void generateDraft()} disabled={generating}><Sparkles className="mr-2 h-4 w-4" />Regenerate from sources</Button></div></div><div className="rounded-2xl border bg-white p-4"><h3 className="font-semibold text-[#082b23]">Automatic document structure</h3><ul className="mt-3 space-y-2 text-xs leading-relaxed text-muted-foreground"><li className="flex gap-2"><Check className="mt-0.5 h-3.5 w-3.5 text-emerald-600" />APAS Consulting cover page</li><li className="flex gap-2"><Check className="mt-0.5 h-3.5 w-3.5 text-emerald-600" />Live table of contents</li><li className="flex gap-2"><Check className="mt-0.5 h-3.5 w-3.5 text-emerald-600" />Readable client typography</li><li className="flex gap-2"><Check className="mt-0.5 h-3.5 w-3.5 text-emerald-600" />Selected photographic record</li><li className="flex gap-2"><Check className="mt-0.5 h-3.5 w-3.5 text-emerald-600" />Controlled source manifest</li></ul></div></aside></div>
          </TabsContent>

          <TabsContent value="preview" className="mt-0">
            <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_300px]"><Card className="overflow-hidden"><div className="flex flex-wrap items-center justify-between gap-3 border-b bg-white px-4 py-3"><div><p className="font-semibold text-[#082b23]">Client-ready preview</p><p className="text-xs text-muted-foreground">Cover, contents, narrative, photographs, and manifest</p></div><div className="flex gap-2"><Button variant="outline" size="sm" onClick={() => void openConsultingReport({ projectName, report: currentReport, sources: sources.data ?? [] }).catch((error) => toast.error(error.message))}><FileDown className="mr-1.5 h-4 w-4" />Open / save PDF</Button><Button size="sm" onClick={() => setEmailOpen(true)}><Mail className="mr-1.5 h-4 w-4" />Email client</Button></div></div><iframe title="Consulting report preview" srcDoc={previewHtml} className="h-[900px] w-full bg-slate-100" /></Card><aside className="space-y-3"><div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4"><div className="flex items-center gap-2 font-semibold text-emerald-950"><CheckCircle2 className="h-4 w-4" />Issue checklist</div><div className="mt-3 space-y-2 text-xs text-emerald-950/75"><p>{conversation.filter((message) => message.role === 'user').length} author narrative entries reviewed</p><p>{includedSources.length} source records included</p><p>{includedSources.filter((source) => source.mime_type?.startsWith('image/')).length} photographs placed</p><p>Long dash characters removed</p></div></div><Button className="w-full" onClick={() => setEmailOpen(true)}><Mail className="mr-2 h-4 w-4" />Issue as HTML + PDF</Button><Button variant="outline" className="w-full" onClick={() => setTab('draft')}><FileText className="mr-2 h-4 w-4" />Return to editor</Button></aside></div>
          </TabsContent>
        </Tabs>
      </main>

      <GoogleDriveDialog open={driveOpen} onOpenChange={setDriveOpen} reportId={report.id} projectId={projectId} />
...
function SourceCard({ source, onUpdate, onRemove }: { source: ConsultingReportSource; onUpdate: (patch: Partial<ConsultingReportSource>) => void; onRemove: () => void }) {
  const isImage = source.mime_type?.startsWith('image/');
  const [caption, setCaption] = useState(source.caption ?? '');
  return <article className={cn('overflow-hidden rounded-2xl border bg-white transition', !source.included && 'opacity-55')}><div className="relative aspect-[16/10] bg-slate-100">{isImage && source.preview_url ? <img src={source.preview_url} alt={source.caption || source.source_name} className="h-full w-full object-cover" /> : <div className="grid h-full place-items-center"><FileText className="h-8 w-8 text-slate-300" /></div>}<div className="absolute left-2 top-2"><Badge className="bg-slate-950/75 text-white backdrop-blur">{source.source_type === 'google_drive' ? <><Cloud className="mr-1 h-3 w-3" />Drive</> : <><Upload className="mr-1 h-3 w-3" />Upload</>}</Badge></div><Button variant="secondary" size="icon" className="absolute right-2 top-2 h-8 w-8" onClick={onRemove}><X className="h-4 w-4" /></Button></div><div className="space-y-3 p-3"><div className="flex items-start gap-2"><Checkbox checked={source.included} onCheckedChange={(value) => onUpdate({ included: value === true })} className="mt-0.5" /><div className="min-w-0"><p className="truncate text-sm font-semibold text-[#082b23]" title={source.source_name}>{source.source_name}</p><p className="text-[10px] uppercase tracking-wide text-muted-foreground">{source.mime_type || 'File'} | {source.size_bytes ? `${Math.max(1, Math.round(source.size_bytes / 1024))} KB` : 'Size unavailable'}</p></div></div>{isImage && <div className="space-y-1.5"><Label className="text-xs">Report caption</Label><Textarea rows={2} value={caption} onChange={(event) => setCaption(removeLongDashes(event.target.value))} onBlur={() => { if (caption !== (source.caption ?? '')) onUpdate({ caption }); }} placeholder="Explain what the client should notice..." /></div>}{source.drive_web_url && <a href={source.drive_web_url} target="_blank" rel="noreferrer" className="inline-flex items-center text-xs font-semibold text-emerald-700 hover:underline">Open original in Drive<ExternalLink className="ml-1 h-3 w-3" /></a>}</div></article>;
}

function GoogleDriveDialog({ open, onOpenChange, reportId, projectId }: { open: boolean; onOpenChange: (open: boolean) => void; reportId: string; projectId: string }) {
  ...
  async function loadFolder() {
    try {
      ...
      setFiles(rows.filter((file) => file.mimeType !== 'application/vnd.google-apps.folder'));
      setSelected(new Set());
  ...
  async function importSelected() {
    ...
    try {
      const result = await drive.importFiles.mutateAsync([...selected]);
