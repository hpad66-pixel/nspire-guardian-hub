...
export function buildConsultingReportHtml({ projectName, report, sources, imageUrls = {} }: ConsultingReportTemplateInput) {
  const included = sources.filter((source) => source.included);
  const images = included.filter((source) => source.mime_type?.startsWith('image/'));
  ...
  const photoPages = images.length ? `<section class="photo-record page-break"><div class="eyebrow">Selected evidence</div><h2>Photographic Record</h2><p class="lead">The following photographs were selected from the reviewed report manifest. Captions may be edited before the report is issued.</p><div class="photo-grid">${images.map((source, index) => `<figure>${imageUrls[source.id] ? `<img src="${escapeHtml(imageUrls[source.id])}" alt="${escapeHtml(source.caption || source.source_name)}">` : '<div class="image-placeholder">Image preview unavailable</div>'}<figcaption><b>Photo ${String(index + 1).padStart(2, '0')}</b><span>${escapeHtml(source.caption || source.source_name)}</span><small>${escapeHtml(source.source_type === 'google_drive' ? 'Imported from Google Drive' : 'Project upload')}</small></figcaption></figure>`).join('')}</div></section>` : '';
...
export function buildConsultingReportEmail(input: ConsultingReportTemplateInput) {
  ...
  const imageCount = included.filter((source) => source.mime_type?.startsWith('image/')).length;
...
export async function secureConsultingReportImages(sources: ConsultingReportSource[]) {
  ...
  const images = sources.filter((source) => source.included && source.mime_type?.startsWith('image/') && source.storage_path);
  const pairs = await Promise.all(images.map(async (source) => {
    const { data } = await supabase.storage.from('project-documents').createSignedUrl(source.storage_path!, 30 * 60);
...
export async function prepareConsultingReportDelivery(input: ConsultingReportTemplateInput) {
  ...
  const printableHtml = buildConsultingReportHtml({ ...input, imageUrls });
...
export async function openConsultingReport(input: ConsultingReportTemplateInput) {
  ...
  const imageUrls = await secureConsultingReportImages(input.sources);
  popup.document.open();
  popup.document.write(buildConsultingReportHtml({ ...input, imageUrls }));
  popup.document.close();
}
