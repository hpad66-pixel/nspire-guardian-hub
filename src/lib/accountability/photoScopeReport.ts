...
export function buildFieldPhotoScopeReport({ projectName, photos, items, imageUrls = {} }: {
  ...
  const disciplineCounts = new Map<ScopeDiscipline, number>();
  groups.flatMap((group) => group.issues).forEach((issue) => {
    classifyScopeIssue(issue).forEach((discipline) => disciplineCounts.set(discipline, (disciplineCounts.get(discipline) ?? 0) + 1));
  });
