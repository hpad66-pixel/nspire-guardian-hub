...
export function buildFieldPhotoScopeEmailText({ projectName, photos, items, personalMessage = '', reportUrl = '' }: {
  ...
  return [personalMessage, `OWNER CONDITION & SCOPE INTELLIGENCE — ${projectName}`, `${photos.length} photographs · ${groups.length} work packages · ${groups.reduce((sum, group) => sum + group.issues.length, 0)} scope items`, reportUrl ? `Live report: ${reportUrl}` : '', ...lines].filter(Boolean).join('\n\n');
}
