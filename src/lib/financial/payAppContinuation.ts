...
// ── Contract & payment position (plain-language summary) ──────────────────────

export interface PaymentPosition {
  baseContract: number;        // original contract sum
  changeOrders: number;        // net approved change orders
  ...
  thisInvoice: number;         // current payment due on this pay app
  paidToDate: number;          // ACTUAL cash received from the client, all pay apps
  outstanding: number;         // earned (less retainage) not yet received in cash
  balanceToBill: number;       // contract value not yet billed
  balanceToFinish: number;     // remaining incl. retainage (revised − earned less retainage)
}

/**
 * Fold the certified G702 cover together with the actual cash received from the
 * client into one plain-language position. `receivedToDate` is the SUM of every
 * recorded receipt on the contract (across all pay apps) — cash in hand, which
 * the G702 itself does not track (its "less previous certificates" is billed,
 * not paid). Every figure is derived, so the money math is unit-tested.
 */
export function computePaymentPosition(g702: G702Summary, receivedToDate: number): PaymentPosition {
  ...
  return {
    baseContract: round2(g702.original_contract_sum),
    changeOrders: round2(g702.net_change_orders),
    revisedContract,
    completedToDate,
    ...
    thisInvoice: round2(g702.current_payment_due),
    paidToDate,
    outstanding: round2(earnedLessRetainage - paidToDate),
    balanceToBill: round2(revisedContract - completedToDate),
    balanceToFinish: round2(g702.balance_to_finish),
  };
}
