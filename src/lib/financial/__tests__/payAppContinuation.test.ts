...
describe("computePaymentPosition", () => {
  const g: G702Summary = {
    ...
    contract_sum_to_date: 547111,        // base + COs
    completed_stored_to_date: 400000,
    retainage_total: 40000,
    total_earned_less_retainage: 360000, // completed − retainage
    less_previous_certificates: 280000,
    current_payment_due: 80000,          // earned less retainage − previous certs
    balance_to_finish: 187111,           // revised − earned less retainage
  };

  it("folds the certified cover + actual cash received into a plain-language position", () => {
    const p = computePaymentPosition(g, 300000); // client has paid $300k in cash
    ...
    expect(p.pctComplete).toBe(73.11); // 400000 / 547111
    ...
    expect(p.outstanding).toBe(60000);     // 360000 earned − 300000 received
    expect(p.balanceToBill).toBe(147111);  // 547111 − 400000
