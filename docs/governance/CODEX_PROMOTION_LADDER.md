...
| 1 | `codex/<ticket-or-date>-<scope>` | Codex workbench | Hardeep + Codex build, inspect, document, and push first drafts. | Codex operator. |
...
| 3 | `release/<date>-<scope>` | QA release candidate | Frozen candidate for QA, final review, release notes, rollback planning. | Tester, DevOps, release owner. |
...
1. Start from current `main`, unless the release owner explicitly says to start
   from `staging`.
2. Create a `codex/<ticket-or-date>-<scope>` branch.
3. Implement the smallest coherent change.
4. Push only to that `codex/*` branch.
5. Open a PR from `codex/*` to `staging`.
...
8. Cut `release/<date>-<scope>` from `staging` when the release set is ready for
   QA freeze.
9. Merge `release/*` to `main` only after QA, DevOps, release notes, rollback
   plan, and Hardeep approval are complete.

## Parallel Work Rule

Parallel work is normal. The protection is that everyone uses the same ladder.

- Codex work starts in `codex/*`.
- Developer work starts in `feature/*` or `fix/*`.
- DevOps work starts in `devops/*`.
...
- A release branch is cut only after the included PR list is known.
...
- `codex/*`, `feature/*`, `fix/*`, `devops/*`: Is this individual change good?
- `staging`: Do multiple approved changes work together?
- `release/*`: Is this exact release candidate ready to go live?
