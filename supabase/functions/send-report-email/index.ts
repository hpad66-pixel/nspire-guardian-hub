...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    if (reportType === "proposal" && body.contentHtml) {
      // ── PROPOSAL-SPECIFIC EMAIL TEMPLATE ──
      const greeting = body.recipientName
        ? `Dear ${body.recipientName},`
        : "Dear Recipient,";

      const instructionsBlock = message
        ? `
          <div style="margin: 24px 0; padding: 20px; background-color: #fffbeb; border: 1px solid #fcd34d; border-radius: 8px;">
            <h3 style="margin: 0 0 8px 0; color: #92400e; font-size: 14px; font-weight: 700;">📋 Action Required / Additional Instructions</h3>
  ...
  <div style="max-width: 700px; margin: 0 auto; padding: 40px 20px;">
    <div style="background-color: #ffffff; border-radius: 12px; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1); overflow: hidden;">
      ...
      <div style="background: linear-gradient(135deg, #1e40af 0%, #3b82f6 100%); padding: 32px; text-align: center;">
        ...
        <p style="margin: 8px 0 0 0; color: rgba(255, 255, 255, 0.9); font-size: 14px;">${propertyName} · ${inspectionDate}</p>
      </div>
      
      <!-- Content -->
      <div style="padding: 32px;">
        <p style="margin: 0 0 20px 0; color: #1e293b; font-size: 15px; line-height: 1.6;">${greeting}</p>
        
        <p style="margin: 0 0 20px 0; color: #475569; font-size: 14px; line-height: 1.6;">
          Please find below the scope of work / proposal for your review. A PDF copy is also attached for your records.
        </p>

        ${instructionsBlock}

        <!-- Proposal Content -->
        <div style="margin: 24px 0; padding: 24px; background-color: #f8fafc; border: 1px solid #e2e8f0; border-radius: 8px;">
          <div style="color: #1e293b; font-size: 14px; line-height: 1.7;">
            ${body.contentHtml}
          </div>
        </div>

        <div style="margin: 24px 0; padding: 16px; background-color: #eff6ff; border-radius: 8px; text-align: center;">
          <p style="margin: 0; color: #1e40af; font-size: 13px; font-weight: 600;">
            📎 A formatted PDF copy of this document is attached to this email.
          </p>
        </div>

        <div style="margin-top: 32px; padding-top: 16px; border-top: 1px solid #e2e8f0;">
          <p style="margin: 0; color: #475569; font-size: 14px;">Regards,</p>
          <p style="margin: 4px 0 0 0; color: #1e293b; font-size: 14px; font-weight: 600;">${inspectorName}</p>
        </div>
      </div>
      ...
      <div style="background-color: #f8fafc; padding: 24px 32px; text-align: center; border-top: 1px solid #e2e8f0;">
        <p style="margin: 0; color: #94a3b8; font-size: 12px;">
          Sent via APAS OS · You may reply directly to this email
        </p>
      </div>
    </div>
  </div>
...
<body style="margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; background-color: #f1f5f9;">
  <div style="max-width: 600px; margin: 0 auto; padding: 40px 20px;">
    <div style="background-color: #ffffff; border-radius: 12px; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1); overflow: hidden;">
      ...
      <div style="background-color: #f8fafc; padding: 24px 32px; text-align: center; border-top: 1px solid #e2e8f0;">
        <p style="margin: 0; color: #94a3b8; font-size: 12px;">
          This report was generated by APAS OS
        </p>
      </div>
    </div>
  </div>
