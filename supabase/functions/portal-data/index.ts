...
serve(async (req) => {
  ...
  try {
    ...
    const trackerUpdates = await grab(async () => {
      ...
      return (await db.from("tracker_updates").select("item_id, author, body, status_to, is_client, photos, created_at")
        .in("item_id", ids).order("created_at", { ascending: false })).data ?? [];
    }, [] as any[]);
    const updByItem: Record<string, any[]> = {};
    (trackerUpdates as any[]).forEach((u) => { (updByItem[u.item_id] ??= []).push({ author: u.author, body: u.body, status_to: u.status_to, is_client: u.is_client, photos: Array.isArray(u.photos) ? u.photos : [], created_at: u.created_at }); });
