-- Structured, trackable meeting-agenda items (one agenda per meeting). Each item
-- is a box you can check off during the meeting; action-item-sourced boxes carry
-- the link + status so they can be color-coded (overdue/due/open).
...
CREATE INDEX IF NOT EXISTS meeting_agenda_items_meeting_idx ON public.meeting_agenda_items (meeting_id, sort_order);

ALTER TABLE public.meeting_agenda_items ENABLE ROW LEVEL SECURITY;
CREATE POLICY meeting_agenda_items_tenant ON public.meeting_agenda_items FOR ALL TO authenticated
