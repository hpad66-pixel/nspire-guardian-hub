-- Parent invoice deletion cascades through consulting_invoice_lines. During that
-- cascade the proposal-billing guard may no longer be able to read the parent
-- row, so treat a missing parent as an allowed DELETE-only cascade. Inserts and
-- updates still require a draft parent invoice and approved proposal authority.
