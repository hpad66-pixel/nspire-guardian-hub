-- Keep consulting invoice edits aligned with the UI lifecycle:
-- - invoice numbers are unique inside a project,
-- - draft invoice numbers may be amended safely,
-- - eligible void invoices can be deleted without the line guard blocking the cascade.

CREATE UNIQUE INDEX IF NOT EXISTS consulting_invoices_project_invoice_no_key
  ON public.consulting_invoices (project_id, invoice_no);
...
BEGIN
  IF TG_OP = 'DELETE' THEN
    ...
    IF invoice_record.status NOT IN ('draft', 'void') THEN
      RAISE EXCEPTION 'Only draft or void invoice lines can be deleted';
    END IF;
